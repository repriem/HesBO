...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 31 08:31:36 2020

@author: ninamoello
"""
#from count_sketch.py 
...
def projection(X, sign, high_to_low):
    low_dim = max(high_to_low) +1 
    X = np.atleast_2d(X)
    n=X.shape[0]
    high_dim=X.shape[1]
    low_obs=np.zeros((n,low_dim))
...
def RunMain(low_dim=2, high_dim=25, initial_n=20, total_itr=100, func_type='Sphere',
    ...
    :param low_dim: The effective dimension of the algorithm. Must be less 
        than or equal to the number of colums of the DOE matrix input
    ...
    To change the DOE please take care of lines 114-125 and 147    
    """
    ...
    if func_type=='Branin':
        ...
        test_func=functions.Quadratic(active_var, noise_var=noise_var)
    elif func_type == 'Sphere':
        test_func=functions.Sphere(active_var, noise_var=noise_var)
    elif func_type == 'Rastrigin':
        test_func=functions.Rastrigin(active_var, noise_var=noise_var)
    ...
    return best_results, elapsed, s, f_s, f_s_true, high_s, low_dim, high_dim, initial_n, total_itr, func_type

if __name__=='__main__':
    res, time, s, f_s, f_s_true, _, low_dim, high_dim, initial_n, total_itr, func_type=RunMain(low_dim=2, 
            high_dim=60, initial_n=5, total_itr=495, func_type='Rover',
            s=np.load('/Applications/anaconda3/envs/HesBO/DOE_HDBO/DOE_5/ROVER_60/DOE_4/doe.npy'), active_var=None, ARD=False, variance=1., length_scale=None, box_size=None,
            high_to_low=None, sign=None, hyper_opt_interval=20, noise_var=0)
    ...
    plt.xlabel('Time')
    plt.ylabel('Result')
    plt.plot(time.T,-res.T,'r')
    plt.show()
    
    #Saving every data in a txt file, the plot in a pdf file and res array in a npy file
    fig.savefig('Plot_HesBO_%s_%i_01' %(func_type,high_dim),format='pdf', dpi=250)
    file = open('Data_HesBO_%s_%i_01' %(func_type,high_dim), 'w')
    file.write('Low dimensional search space in dim : %i \r High dimensional search space in dim :%i \r Number of initial points :%i \r Number of iterations of algorithm :%i' %(low_dim,high_dim,initial_n,total_itr))
    file.write("\r Result: %a" %-res)
    file.write("\r Time: %a" %time)
    file.close()

    np.save('fX_HesBO_%s_%i_01' %(func_type,high_dim), -res)
