...
from moe.optimal_learning.python.cpp_wrappers.domain import (
    TensorProductDomain as cppTensorProductDomain,
)
...
from moe.optimal_learning.python.python_version.domain import (
    TensorProductDomain as pythonTensorProductDomain,
)
...
from moe.optimal_learning.python.python_version.optimization import (
    multistart_optimize as multistart_optimize,
)
...
obj_func_dict = {
    "Branin": Branin(act_var, low_dim, high_to_low, sign, bx_size, noise_var=noise_var),
...
for pt in init_pts:
    pt[objective_func._dim - objective_func._num_fidelity :] = np.ones(
...
for n in range(num_iteration):
    ...
    if method == "KG":
        ...
        discrete, _ = bayesian_optimization.gen_sample_from_qei_mcmc(
            cpp_gp_loglikelihood._gaussian_process_mcmc,
            cpp_search_domain,
            cpp_sgd_params_kg,
            10,
            num_mc=2 ** 10,
    ...
    sampled_points = [
        SamplePoint(
            pt, objective_func.evaluate(pt)[observations], objective_func._sample_var
        )
    ...
    if method == "KG":
        ...
        initial_point = np.reshape(
            [eval_pts[t] for t in top_k],
            (num_opt_start, cpp_gp_loglikelihood.dim - objective_func._num_fidelity),
        )

        py_repeated_search_domain = RepeatedDomain(
            num_repeats=1, domain=inner_search_domain
        )
        ps_mean_opt = pyGradientDescentOptimizer(
            py_repeated_search_domain, ps, py_sgd_params_ps
        )
        report_point = multistart_optimize(
            ps_mean_opt, initial_point, num_multistarts=num_opt_start
        )[0]
        ...
        if -ps.compute_objective_function() > np.min(test):
            report_point = initial_point[0].reshape(
                (1, cpp_gp_loglikelihood.dim - objective_func._num_fidelity)
            )
    else:
        cpp_gp = cpp_gp_loglikelihood.models[0]
        report_point = (cpp_gp.get_historical_data_copy()).points_sampled[
            np.argmin(cpp_gp._points_sampled_value[:, 0])
        ]
    ...
    data_str = (
        data_str
        + "Top_k values of mean posterior:"
        + str([test[t] for t in top_k])
        + "\n"
    )
    data_str = (
        data_str
        + "True values of suggested points:"
        + str(objective_func.evaluate_true(s_suggest))
        + "\n"
    )
    data_str = data_str + "Noisy values of suggested points:" + str(f_s_suggest) + "\n"
    data_str = (
        data_str + "True values of recommended points:" + str(f_s_recommend) + "\n"
    )
