...
from moe.optimal_learning.python.cpp_wrappers.expected_improvement import (
    ExpectedImprovement as cppExpectedImprovement,
)
from moe.optimal_learning.python.cpp_wrappers.expected_improvement import (
...
from moe.optimal_learning.python.cpp_wrappers.expected_improvement_mcmc import (
    ExpectedImprovementMCMC as cppExpectedImprovementMCMC,
)

from moe.optimal_learning.python.cpp_wrappers.knowledge_gradient_mcmc import (
    KnowledgeGradientMCMC as cppKnowledgeGradientMCMC,
)
from moe.optimal_learning.python.cpp_wrappers.knowledge_gradient_mcmc import (
...
def gen_sample_from_qei(
    cpp_gp, cpp_search_domain, sgd_params, num_to_sample, num_mc=1e4, lhc_itr=2e4
):
    """
    ...
    """
    cpp_ei_evaluator = cppExpectedImprovement(
        gaussian_process=cpp_gp, num_mc_iterations=int(num_mc)
    )
    ...
    optimizer = cppGradientDescentOptimizer(
        cpp_search_domain, cpp_ei_evaluator, sgd_params, int(lhc_itr)
    )
...
def gen_sample_from_qei_mcmc(
    cpp_gp_mcmc, cpp_search_domain, sgd_params, num_to_sample, num_mc=1e4, lhc_itr=2e4
):
    """
    ...
    points_to_sample_list.append(
        multistart_expected_improvement_mcmc_optimization(
            optimizer, None, num_to_sample=num_to_sample, max_num_threads=1
        )
    )
    cpp_ei_evaluator.set_current_point(points_to_sample_list[0])
...
def gen_sample_from_qkg_mcmc(
    ...
    :return: (points to sample next, expected improvement at this set of points)
    """
    cpp_kg_evaluator = cppKnowledgeGradientMCMC(
        gaussian_process_mcmc=cpp_gp_mcmc,
        gaussian_process_list=cpp_gp_list,
    ...
    optimizer = cppGradientDescentOptimizer(
        cpp_search_domain, cpp_kg_evaluator, sgd_params, int(lhc_itr)
    )
    points_to_sample_list = []
    kg_list = []
