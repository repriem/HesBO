...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  1 11:36:32 2020
...
def RunRembo(low_dim=2, high_dim=20, initial_n=20, total_itr=100, func_type='Branin',
    ...
    :param low_dim: the dimension of low dimensional search space, must be 
        equal to the number of colums of the DOE matrix input
    ...
    A = matrix.evaluate()
    np.save('Matrix_RemBOY_%s_%i_01' %(func_type,high_dim), A)
    ...
    return best_results, elapsed, s, f_s, f_s_true, cnv_prj.evaluate(s), low_dim, high_dim, initial_n, total_itr, func_type

if __name__=='__main__':
    res,time, s, f_s, fs_true, high_s,low_dim, high_dim, initial_n, total_itr, func_type =RunRembo(low_dim=2, 
                                    high_dim=60, func_type='Rover', initial_n=120,
                                    s=np.load('/Applications/anaconda3/envs/HeSBO/DOE_HDBO/DOE_2D/ROVER_60/DOE_4/doe.npy'),
                                    total_itr=380, kern_inp_type='Y', ARD=True, noise_var=1)
    print(-res,time)
    ...
    fig.savefig('Plot_RemBOY_%s_%i_01' %(func_type,high_dim),format='pdf', dpi=250)
    file = open('Data_RemBOY_%s_%i_01' %(func_type,high_dim), 'w')
    ...
    np.save('fX_RemBOY_%s_%i_01' %(func_type,high_dim), -res)
